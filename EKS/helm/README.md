...
Use this helm chart to customise your install of Mavic Drone Shop.

### Helm v2.x

```shell
$ helm install --name mavic-drone-shop --namespace mavic-drone-shop .
```

### Helm v3.x

```bash
$ kubectl create ns mavic-drone-shop
$ helm install mavic-drone-shop --namespace mavic-drone-shop .
```
...
```shell
$ helm install mavic-drone-shop --set openshift=true helm
```
...
| image.repo       | mavicdroneshop | string | Base docker repository to pull the images from. |
...
> `<workload>` can be substituted with the different microservices consisting of Mavic Drone Shop, namely:
...
Hence, this way we can control which `Mavic Drone Shop` workloads are running on which nodes/nodepools.
